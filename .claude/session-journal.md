...
- Guides generated by committed scripts/gen-reading-universes.mjs (Solaria, SQUIB S&S/BTV tandem w/ completeness asserts, TOG, ACOTAR, CC, Maasverse). EOS/ToD fixed: Nightfall/Fireheart are prologue/epilogue, not bonus/novellas; credit to Jenna Clare. Existing item ids preserved everywhere (checked with comm).
- **Decision:** official Solaria maps are link-out cards, not hosted copies — Gavin believed "no usage terms = usable"; told him no terms = all rights reserved by default. Gallery supports kind:"image" (src under /public) so hosting is a one-line data change if he still wants it.
- Gotcha: right after a CI deploy some new routes 404 for ~a minute while the Worker version propagates — re-check before debugging.
